...
// Type-safe form field component - Made smaller
...
const CustomerAddUpdate: React.FC<CustomerAddUpdateProps> = ({ 
  ...
  // FIX: Add state to track current customer ID
  ...
  // Reset form function - COMPLETELY reset all fields EXCEPT customer number
  const resetForm = (newCustomerNumber: string) => {
    // Reset react-hook-form with the new customer number
    reset({
      branch_id: user?.branch || undefined,
      created_by: user?.id || 'system',
      last_updated_by: user?.id?.toString() || 'system',
      status: 'active',
      customer_number: newCustomerNumber, // Keep the new generated number
      ...
      signature: '',
      telephone_number: '',
      next_of_kin_age: undefined,
      next_of_kin_occupation: '',
      next_of_kin_gender: '' as any,
      next_of_kin_address: '',
      next_of_kin_telephone_number: '',
    ...
    setCurrentCustomerId(undefined); // FIX: Clear current customer ID
    setCustomerNumber(newCustomerNumber); // Set the new customer number
    setCustomerFullName('');
    setAccounts([]);
    setImagePreview('');
    setSignaturePreview('');
  ...
  const generateNewCustomerNumber = async () => {
    ...
    try {
      ...
      // Reset form with the new customer number
      resetForm(newNumber);
    } catch (error) {
      console.error('Error generating customer number:', error);
      const fallbackNumber = generateRandomCustomerNumber();
      // Reset form with the fallback customer number
      resetForm(fallbackNumber);
    } finally {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      let response;
      // FIX: Use currentCustomerId for updates instead of trying to search again
      if (customerId || isExistingCustomer) {
        // Use PUT for updates - use the current customer ID
        const updateCustomerId = currentCustomerId || customerId;
        if (!updateCustomerId) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="text-sm">
        ...
        <div className="flex items-center gap-2 mb-2">
          <div className="flex-1">
            ...
            <div className="flex gap-1">
              <input
                ...
                className="flex-1 p-0 px-1.5 text-sm border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
              ...
              <a
                ...
                className="px-2 py-0 bg-blue-600 text-white cursor-pointer  hover:bg-blue-700 flex items-center gap-1 text-sm"
              >
               ...
               {!customerId && (
                <button
                  ...
                  className="px-2 py-0 btn-sm bg-green-600 text-white  hover:bg-green-700 disabled:opacity-50 flex items-center gap-1 text-sm"
                  title="Generate new number"
                >
                  <RefreshCw className={`h-3 w-3 ${generatingNumber ? 'animate-spin' : ''}`} /> New
                </button>
              )}
            </div>
          </div>
          <div className="flex-1">
            ...
            <input
              ...
              disabled
              className="w-full p-1.5 text-sm border border-gray-300 rounded bg-gray-100 text-gray-600"
            />
          </div>
        </div>
      </div>

      {/* Tabs Section - Made smaller */}
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        <div className="">
          <div className="flex bg-blue-50 p-0 rounded-t-lg">
            ...
            <a
              ...
              className={`flex-1 py-3 px-3 text-center font-medium  cursor-pointer transition-colors ${
                activeTab === 'details'
        ...
        <div className="p-3">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {activeTab === 'details' && (
              <div className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    {errors.customer_number && (
                      <p className="mt-1 text-xs text-red-600">{errors.customer_number.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      {...register("title")}
                      className="w-full p-1.5 text-sm border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500"
                  ...
                  <div>
                    ...
                    {errors.telephone_number && (
                      <p className="mt-1 text-xs text-red-600">{errors.telephone_number.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">GPS Address</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Home Town</label>
                    <input
                      type="text"
                      {...register("home_town")}
                      className="w-full p-1.5 text-sm border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500"
                ...
                <div>
                  ...
                  />
                  {errors.address_one && (
                    <p className="mt-1 text-xs text-red-600">{errors.address_one.message}</p>
                  )}
                </div>
                ...
                <div className="border-t pt-3">
                  <h4 className="text-sm font-medium text-gray-900 mb-2">Next of Kin Information</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Next of Kin Name</label>
                      <input
                        type="text"
            ...
            {/* Accounts Tab - Only for existing customers */}
            {/* FIX: Use currentCustomerId instead of the complex fallback logic */}
            {activeTab === 'accounts' && isExistingCustomer && currentCustomerId && (
              <CustomerAccounts
            ...
            {activeTab === 'identification' && isExistingCustomer && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <h4 className="text-sm font-medium text-gray-900 mb-2">Customer Picture</h4>
                  <div className="space-y-2">
                    <div className="flex gap-1">
                      <button type="button" onClick={() => setCaptureMode('upload')} className={`px-2 py-1.5 rounded flex items-center gap-1 text-xs ${captureMode === 'upload' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
                        <Upload className="h-3 w-3" /> Upload
                      </button>
                      <button type="button" onClick={() => setCaptureMode('camera')} className={`px-2 py-1.5 rounded flex items-center gap-1 text-xs ${captureMode === 'camera' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
                    ...
                    {captureMode === 'camera' && (
                      <div className="border-2 border-dashed border-gray-300 rounded p-4 text-center">
                        ...
                        <p className="text-gray-500 text-xs">Camera functionality</p>
                      </div>
                    )}
                    ...
                    {imagePreview && (
                      <div className="mt-2">
                        ...
                        <img src={imagePreview} alt="Preview" className="w-24 h-24 object-cover rounded border border-gray-300" />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {signaturePreview && (
                      <div className="mt-2">
                        <p className="text-xs font-medium text-gray-700 mb-1">Preview:</p>
                        <img src={signaturePreview} alt="Signature" className="w-36 h-18 object-contain border border-gray-300 bg-white" />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="md:col-span-2">
                  <h4 className="text-sm font-medium text-gray-900 mb-2">Identification Information</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">SSN/Social Security</label>
                      <input
                        type="text"
                        {...register("SSNumber")}
                        className="w-full p-1.5 text-sm border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500"
      ...
      {/* Search Modal - Made smaller */}
      <AnimatePresence>
      {searchModalOpen && (
        <div className="fixed inset-0 bg-black   flex items-start justify-center p-4 z-[1811]">
          <motion.div initial={{ opacity: 0, scale: 1 }} animate={{ opacity: 1, scale: 1 }} className="bg-white w-full max-w-2xl max-h-[80vh] overflow-hidden text-sm">
            <div className="p-3 border-b border-gray-200">
              <h3 className="text-md font-medium text-gray-900">Search Customer</h3>
            </div>
            
            <div className="p-3">
              <input type="text" value={searchQuery} onChange={(e) => handleNameSearch(e.target.value)} placeholder="Search customers..." className="w-full p-1.5 text-sm border border-gray-300 rounded mb-3" />
              
              <div className="max-h-64 overflow-y-auto">
                <table className="min-w-full divide-y divide-gray-200 text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase">Customer #</th>
                      <th className="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase">First Name</th>
                      <th className="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase">Last Name</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {searchResults.map((customer) => (
                      <tr key={customer.id} className="hover:bg-gray-50">
                        <td className="px-3 py-1.5 text-sm font-medium text-gray-900">{customer.customer_number}</td>
                        <td className="px-3 py-1.5 text-sm text-gray-500">{customer.first_name}</td>
                        <td className="px-3 py-1.5 text-sm text-gray-500">{customer.last_name}</td>
                        <td className="px-3 py-1.5 text-sm font-medium">
                          <button onClick={() => selectCustomer(customer)} className="text-blue-600 hover:text-blue-900 text-xs">Select</button>
                        </td>
                      </tr>
                    ))}
                    {searchResults.length === 0 && searchQuery.length >= 2 && !searchLoading && (
                      <tr><td colSpan={4} className="px-3 py-3 text-center text-sm text-gray-500">No customers found</td></tr>
                    )}
                    {searchLoading && <tr><td colSpan={4} className="px-3 py-3 text-center text-sm text-gray-500">Searching...</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
            ...
            <div className="px-3 py-2 bg-gray-50 border-t border-gray-200 flex justify-end">
              <a onClick={() => setSearchModalOpen(false)} className="px-2 py-1 cursor-pointer bg-gray-300 text-gray-700 text-sm hover:bg-gray-400">Close</a>
            </div>
          </motion.div>
        </div>
      )}
